...
exports.getCotisation = asyncHandler(async (req, res, next) => {
  ...
  if (!cotisation) {
    return next(
      new ErrorResponse(
        `Cotisation non trouvée avec l'id ${req.params.id}`,
        404
      )
    );
  }
  ...
  if (
    cotisation.membre._id.toString() !== req.user.id &&
    req.user.role !== "admin"
  ) {
    return next(
      new ErrorResponse(
        `L'utilisateur ${req.user.id} n'est pas autorisé à accéder à cette cotisation`,
        403
      )
    );
  }
...
exports.createCotisation = asyncHandler(async (req, res, next) => {
  ...
  if (existingCotisation) {
    return next(
      new ErrorResponse(
        `Vous avez déjà une cotisation pour ${req.body.mois} ${req.body.annee}`,
        400
      )
    );
  }
...
exports.updateCotisation = asyncHandler(async (req, res, next) => {
  ...
  if (
    cotisation.membre.toString() !== req.user.id &&
    req.user.role !== "admin"
  ) {
    return next(
      new ErrorResponse(
        `L'utilisateur ${req.user.id} n'est pas autorisé à mettre à jour cette cotisation`,
        403
      )
    );
  }
  ...
  if (cotisation.statut !== "En attente") {
    return next(
      new ErrorResponse(
        `Seules les cotisations en attente peuvent être modifiées`,
        400
      )
    );
  }
...
exports.updateCotisationStatus = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `L'utilisateur n'est pas autorisé à changer le statut des cotisations`,
        403
      )
    );
  }
...
exports.deleteCotisation = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorResponse(
        `L'utilisateur ${req.user.id} n'est pas autorisé à supprimer cette cotisation`,
        403
      )
    );
  }
  ...
  if (cotisation.statut !== "En attente" && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `Seules les cotisations en attente peuvent être supprimées`,
        400
      )
    );
  }
...
exports.getCotisationsByMembre = asyncHandler(async (req, res, next) => {
  ...
  if (membreId !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `L'utilisateur n'est pas autorisé à accéder aux cotisations d'un autre membre`,
        403
      )
    );
  }

  const cotisations = await Cotisation.find({ membre: membreId }).sort(
    "-annee -mois"
  );
...
exports.generateReport = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `L'utilisateur n'est pas autorisé à générer des rapports`,
        403
      )
    );
  }
  ...
  const cotisationsConfirmees = cotisations.filter(
    (c) => c.statut === "Confirmé"
  ).length;
  const cotisationsEnAttente = cotisations.filter(
    (c) => c.statut === "En attente"
  ).length;
  const cotisationsRejetees = cotisations.filter(
    (c) => c.statut === "Rejeté"
  ).length;
