...
const groupeService = {
  ...
  getGroupesByMembre: async (membreId) => {
    const response = await api.get(`/groupes/membre/${membreId}`);
    return response.data;
  },
  ...
  updateGroupe: async (id, groupeData) => {
    const response = await api.put(`/groupes/${id}`, groupeData);
    return response.data;
  },
  ...
  addMembreToGroupe: async (groupeId, userId) => {
    const response = await api.put(`/groupes/${groupeId}/membres/${userId}`);
    return response.data;
  },

  // Retirer un membre d'un groupe
  removeMembreFromGroupe: async (groupeId, userId) => {
    const response = await api.delete(`/groupes/${groupeId}/membres/${userId}`);
    return response.data;
  },
};
